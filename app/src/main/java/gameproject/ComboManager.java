...
public class ComboManager {
    ...
    private final long COMBO_DURATION = 3600; // Chuẩn 3.6 giây
    ...
    public float getFireRateBonus() {
        if (comboCount >= TIER_3)
            return 0.10f; // Tier 3: +10%
        if (comboCount >= TIER_2)
            return 0.05f; // Tier 2: +5%
        return 0f;
    }

    public float getMoveSpeedBonus() {
        if (comboCount >= TIER_3)
            return 0.20f; // Tier 3: +20%
        if (comboCount >= TIER_2)
            return 0.15f; // Tier 2: +15%
        if (comboCount >= TIER_1)
            return 0.10f; // Tier 1: +10%
        return 0f;
    }
    ...
    public Color getComboColor() {
        if (comboCount >= TIER_3)
            return new Color(255, 140, 0); // Orange-Gold rực cháy
        if (comboCount >= TIER_2)
